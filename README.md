...
- `templates/mcdreforged/`: tracked defaults for `config.yml` and `permission.yml` (installers copy from here when available).
- `server/`: created by the install scripts; contains Fabric server files, `mods/`, `downloads/`, and runtime output.

## MCDReforged setup

- The install scripts will create a minimal config at `server/config/config.yml` if none exists (or copy from `server/config/mcdreforged/config.yml`), setting `start_command` from your `config/server.json` memory/flags. They also create `server/config/permission.yml` (or copy from `server/config/mcdreforged/permission.yml`). If `templates/mcdreforged/` exists, those files are copied first and `__START_COMMAND__` is replaced.
